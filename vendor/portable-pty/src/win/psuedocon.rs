    ...
    pub fn new(size: COORD, input: FileDescriptor, output: FileDescriptor) -> Result<Self, Error> {
        ...
        let result = unsafe {
            (CONPTY.CreatePseudoConsole)(
                ...
                // Boite: no PSUEDOCONSOLE_INHERIT_CURSOR. With it the console
                // host asks the terminal for the cursor position and runs
                // nothing until it is answered, and ClosePseudoConsole does not
                // break that wait: a pane closed before its terminal answered
                // left a hung conhost, and a reader thread that never ended.
                // Every Boite terminal is a reset xterm, so there is no cursor
                // to inherit.
